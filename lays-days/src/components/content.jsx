import React from "react";

export default class Content extends React.Component {
  render() {
    let contentStyles = "content";
    let slideToLeft = " wow slideToLeft";
    let slideToRight = " wow slideToRight";
    let animationZoom = " wow animationZoom";
    let fadeIn = " wow fadeIn";
    let contentSection3Text = (
      <p className="text">
        Этим летом брэнд Lay's задает тренд на яркое,
        <br/>
        вкусное лето и на позитивный желтый цвет.
      </p>
    );
    let footer = (
      <div className="footer">
          <div className="icon" />
          <p className="text">© 2015 Lay's Russia. Все права защищены</p>
          <div className="button__block">
            <a href="https://www.facebook.com/lays.russia" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,)",
                  "background-color": "#336AA3"
                }}
              />
            </a>
            <a href="https://vk.com/lays_russia" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,",
                  "background-color": "#307AA9"
                }}
              />
            </a>
            <a href="https://lays.ru/" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,)",
                  "background-color": "#EEBF11"
                }}
              />
            </a>
            <a href="https://www.youtube.com/user/LaysRussia" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,)",
                  "background-color": "#C4070E"
                }}
              />
            </a>
          </div>
        </div>
    )
    if (this.props.widthScreen <= 680) {
      slideToLeft = "";
      slideToRight = "";
      animationZoom = "";
      fadeIn = "";
      contentSection3Text = (
        <p className="text_mobile">
          Этим летом брэнд
          <br/>
          Lay's задает тренд
          <br/>
          на яркое, вкусное
          <br/>
          лето и на позитивный
          <br/>
          желтый цвет.
      </p>
      );
      footer = (
        <div className="footer">
          <p className="text">© 2015 Lay's Russia</p>
          <div className="button__block">
            <a href="https://www.facebook.com/lays.russia" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,)",
                  "background-color": "#336AA3"
                }}
              />
            </a>
            <a href="https://vk.com/lays_russia" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,",
                  "background-color": "#307AA9"
                }}
              />
            </a>
            <a href="https://lays.ru/" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,)",
                  "background-color": "#EEBF11"
                }}
              />
            </a>
            <a href="https://www.youtube.com/user/LaysRussia" target="blank">
              <div
                className="button"
                style={{
                  "background-image":
                    "url(data:image/jpeg;base64,)",
                  "background-color": "#C4070E"
                }}
              />
            </a>
          </div>
        </div>
      )
    }
    if (this.props.viewMenu === false) {
      contentStyles = "content scrollDown";
    }

    return (
      <div className={contentStyles}>
        <section className={"content-section-1" + slideToLeft}>
          <div className="item-section-1">
            <div className={"icon" + animationZoom} />
            <p className={"text" + fadeIn}>Загружай фото</p>
            <p className={"text" + fadeIn}>с хэштегом</p>
            <p className={"text hashtag" + fadeIn}>#летосLays</p>
          </div>
          <div className="item-section-2">
            <div className={"icon" + animationZoom} />
            <p className={"text" + fadeIn}>Собирай</p>
            <p className={"text" + fadeIn}>лайки</p>
          </div>
          <div className="item-section-3">
            <div className={"icon" + animationZoom} />
            <p className={"text" + fadeIn}>Выигрывай</p>
            <p className={"text" + fadeIn}>призы</p>
          </div>
          <div className="text-block">
            <h1 className="first">ЛЕТО</h1>
            <h1 className="middle">ВКУСНЕЕ</h1>
            <h1 className="last">
              С <span>Lay's</span>
            </h1>
          </div>
        </section>
        <section className={"content-section-2" + slideToRight}>
          <div className="content-section-2-1">
            <div className="image" />
            <div className="text">
              <p>Загружай</p>
              <p>фото</p>
              <p>с хэштегом </p>
              <p className="hashtag">#летосLays</p>
            </div>
            <div className="button">ГАЛЕРЕЯ</div>
          </div>
          <div className="content-section-2-2">
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/1.dc59205a.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/2.09d5f843.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/3.416330f5.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/4.71d17a4f.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/5.d0f1531e.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/6.d542dd6f.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/7.7c6c40b0.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/8.0a2ab336.png)",
                "background-size": "cover"
              }}
            />
            <div
              style={{
                "background-image":
                  "url(https://pepsico-lays-yellow-trend.snpdev.ru/images/stub/9.4c9c4447.png)",
                "background-size": "cover"
              }}
            />
          </div>
        </section>
        <section className="content-section-3">
          <div className={"content-section-3-1" + slideToRight}>
            <div className="image" />
            <div className="title">
              <p className="title_text">Выиграй призы</p>
              <p className="title_text">от Lay's</p>
            </div>
            {contentSection3Text}
            <div className="button">О ПРОЕКТЕ</div>
          </div>
          <div className={"content-section-3-2" + slideToLeft}>
            <div className="image" />
          </div>
        </section>
        <section className="content-section-4">
          <div className={"content-section-4-1" + slideToRight}>
              <div className="image"></div>
          </div>
          <div className={"content-section-4-2" + slideToLeft}>
            <p className="title_text">Лето с Lay's</p>
            <div className="button">ПЕРЕЙТИ</div>
          </div>
        </section>
        {footer}
      </div>
    );
  }
}
